...
files.forEach(file => {
    ...
    if (file.includes('DashboardSidebar')) {
        content = content.replace(
            /<p className="px-3 text-(?:xs|sm) font-semibold uppercase tracking-\[0\.28em\] text-slate-500">/g,
            '<p className="text-xs uppercase tracking-widest text-sidebar-foreground/60 font-black px-3 mb-1">'
        );
    }

    // Protect our new SidebarGroupLabel which has text-xs from being bumped
    content = content.replace(/\btext-sidebar-foreground\/60 font-black px-3 mb-1\b/g, '__PROTECTED_GROUP_LABEL__');

    // Protect some Tailwind prefixes like context- etc, but \btext- handles mostly cleanly. 
    // Except when inside a string block.
    ...
    // Restore protected group label
    content = content.replace(/__PROTECTED_GROUP_LABEL__/g, 'text-sidebar-foreground/60 font-black px-3 mb-1');
